...
import { ControlChange, ControlChangeInterface, ControlChangeJSON } from './ControlChange';
...
export class Track {
	...
	constructor(trackData: MidiTrackData, public readonly header: Header) {
	...
	toJSON(): TrackJSON {
		...
		const controlChanges: { [key: number]: ControlChangeJSON[] } = {};
		for (let i = 0; i < 127; i++) {
	...
	copy(notes: Note[], channel: number): Track {
		const t = new Track([], this.header);
		t.channel = channel;
		t.controlChanges = this.controlChanges;
		t.instrument = this.instrument;
